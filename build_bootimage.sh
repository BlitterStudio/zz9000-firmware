...
if [ "$PRELOAD_BOOTROM" -eq 1 ]; then
    if ! command -v "${CC_FOR_BUILD:-cc}" >/dev/null 2>&1; then
        echo "ERROR: host C compiler not found. Set CC_FOR_BUILD=/path/to/cc." >&2
        exit 1
    fi

    TMP_BOOTROM_TOOL=$(mktemp "${TMPDIR:-/tmp}/zz9000-bootrom-tool.XXXXXX")

    "${CC_FOR_BUILD:-cc}" -Wall -Wextra -Werror -std=c99 \
        -o "$TMP_BOOTROM_TOOL" util/make_bootrom_image.c
    "$TMP_BOOTROM_TOOL" "$BOOTROM_IMAGE"
fi
...
if [ "$PRELOAD_BOOTROM" -eq 1 ]; then
cat <<EOF
  [load=$BOOTROM_LOAD_ADDRESS] $BOOTROM_IMAGE
EOF
fi
cat <<EOF
  $BITSTREAM
